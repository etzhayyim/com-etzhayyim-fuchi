(ns fuchi.methods.rail-housing-commons
  "rail_housing_commons.cljc — housing-commons (LANDS.md) single-rail R1 → gated-live DESIGN.

  Multi-gen first: stable housing floor for 子・孫 wellbecoming (not a personal score).
  Provider is commons-land (commons), not cash rent. live=false; cash≡0;
  disclosure held → refuse. Portable .cljc."
...
(def RAIL-KIND "housing-commons")
(def PROVIDER-DID "commons-land")
(def PRIORITY-STACK pp/PRIORITY-STACK)

(def MULTI-GEN-FACTS
  ["housing-floor-supports-ko-and-mago-households"
...
#?(:clj
   (defn load-design []
     (let [actor (or (System/getenv "FUCHI_ACTOR_DIR")
                     (io/file "."))]
       (edn/load-edn (io/file actor "data" "rail-housing-commons-design.edn")))))
...
(defn housing-rail [imputed]
  {:kind RAIL-KIND :imputed-usd-micros-yr (long imputed) :member-principal false})

(defn r1-dry-intent [alloc-id imputed]
  (let [i (first (provision/provision [(housing-rail imputed)] alloc-id))]
    (when-not (= RAIL-KIND (:rail-kind i))
      (throw (ex-info "expected housing-commons" {:got i})))
    (assert-invariants! i)
    (when-not (= PROVIDER-DID (:provider-did i))
      (throw (ex-info "provider must be commons-land" {:p (:provider-did i)})))
    i))
...
(defn r1-dry-package
  [{:keys [alloc-id subject-did imputed-usd-micros-yr person hold-machine]
    :or {alloc-id "alloc-housing" imputed-usd-micros-yr 0}}]
...
(defn gated-live-plan [r1-pkg gate & {:keys [env hold-machine]}]
  ...
  (let [ds (:disclosure-state r1-pkg)
    ...
    ;; Housing / new commons-land grant is invariant-adjacent → Council Lv7+ in live.
    ...
    (let [authorized (first (provision/dispatch-live [(:intent r1-pkg)] gate :env env))
          pkg (assoc r1-pkg :phase :gated-live-plan :authorized-to-publish true
                     ...
                     :note "authorized plan only — commons-land grant not invoked")]
...
(defn gated-live-status
  "Non-raising R1→gated-live DESIGN status for housing-commons (子・孫 multi-gen).
   Default gate/env refuses. land-grant-executed always false (Council/G10).
   cash≡0. live false. no scores."
  [r1-pkg & {:keys [gate env hold-machine council-housing-held?]}]
  (cond
    (nil? r1-pkg)
    nil
    ...
    {:rail-kind RAIL-KIND
     ...
     :council-housing-held? (boolean council-housing-held?)
     :live false
     :cash-usd-micros 0
     :score-surface []
     :priority-stack PRIORITY-STACK
     :note "R1 refused — gated-live not attempted; land grant false"}

    (true? council-housing-held?)
    {:rail-kind RAIL-KIND
     :provider-did PROVIDER-DID
     :phase :refused
     ...
     :authorized-to-publish false
     :land-grant-executed false
     :refusal-reason "housing-held-awaiting-council-lv7"
     :disclosure-state (or (:disclosure-state r1-pkg) "open")
     :council-housing-held? true
     :live false
     :cash-usd-micros 0
     :score-surface []
     :priority-stack PRIORITY-STACK
     :note "R1 dry ok; housing held under Council — land grant not executed"}
      ...
      (try
        (let [plan (gated-live-plan r1-pkg g :env e :hold-machine hold-machine)
              out {:rail-kind RAIL-KIND
                   ...
                   :score-surface []
                   :priority-stack PRIORITY-STACK
                   :note "gated-live plan authorized — commons-land grant still not invoked"}]
        ...
        (catch #?(:clj Exception :cljs :default) ex
          (let [st (live-gate/gate-status g e)
                out {:rail-kind RAIL-KIND
                     ...
                     :score-surface []
                     :priority-stack PRIORITY-STACK
                     :note "R1 dry ok; gated-live refused; land grant false"}]
            (pp/assert-no-public-scores! out)
            out))))))
