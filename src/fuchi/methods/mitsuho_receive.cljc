...
(defn gated-receive-status
  "Non-raising R1→gated-receive DESIGN status for mitsuho (priority rail #3).
   Default gate/env refuses. Never invokes produce; cash≡0; live=false."
  ...
  (cond
    ...
    (let [out {:rail-kind "food-mitsuho"
               :provider-did PROVIDER-DID
               :phase :refused
               :r1-phase :refused
               :admissible false
      ...
      (try
        (let [ack (gated-receive-plan r1-pkg g :env e)
              out {:rail-kind "food-mitsuho"
                   ...
                   :score-surface []
                   :priority-stack PRIORITY-STACK
                   :note "gated-receive plan authorized — mitsuho produce not invoked"}]
          (pp/assert-no-public-scores! out)
          out)
