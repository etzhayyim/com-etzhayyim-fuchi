...
#?(:clj (def ^:private root-dir actor-dir))
...
#?(:clj (defn- manifest [] (:actor/manifest (canonical-edn/read-string (slurp manifest-file)))))
...
(deftest test-manifest-lexicons-match-lex-files
  #?(:clj
     (let [declared (set (map #(get % "id") (get (manifest) "lexiconNamespaces")))
       ...
       (is (clojure.set/subset? declared files)
           (str "manifest declarations missing from files: " (clojure.set/difference declared files))))))
