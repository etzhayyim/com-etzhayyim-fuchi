(ns fuchi.methods.test-liquidity-warifu-receive
  (:require [clojure.test :refer [deftest is]]
            [fuchi.methods.liquidity-warifu-receive :as wr]
...
(defn- person []
  {:did "did:web:etzhayyim.com:member:seth" :covenant "vowed"
   :rails [{:kind "liquidity" :active? true}] :floor-usd-micros-yr 1000
   :disclosure fresh :exit-suspended? false})

(deftest test-dry-receive
  (let [pkg (w/r1-dry-package {:alloc-id "a" :imputed-usd-micros-yr 1000 :person (person)})
        ack (wr/receive-from-r1-package pkg)]
    (is (= :dry-ack (:phase ack)))
    (is (true? (:member-principal ack)))
    (is (false? (:loan-invoked ack)))
    (is (false? (:live ack)))
    (is (= 0 (:cash-usd-micros ack)))
    (pp/assert-no-public-scores! ack)))

(deftest test-gated-no-loan
  (let [pkg (w/r1-dry-package {:alloc-id "a" :imputed-usd-micros-yr 1 :person (person)})
        gate (live-gate/make-live-gate
              {:leg "provision" :operator-did "did:op:x" :council-level 6
               :member-signature "member-cap-ok"})
        ack (wr/gated-receive-plan pkg gate :env {"FUCHI_ALLOW_LIVE_PROVISION" "1"})]
