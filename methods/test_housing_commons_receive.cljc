(ns fuchi.methods.test-housing-commons-receive
  (:require [clojure.test :refer [deftest is]]
            [fuchi.methods.housing-commons-receive :as hr]
...
(defn- person []
  {:did "did:web:etzhayyim.com:member:abel" :covenant "vowed"
   :rails [{:kind "housing" :active? true}] :floor-usd-micros-yr 1000
...
(deftest test-dry-receive
  (let [pkg (h/r1-dry-package {:alloc-id "a" :imputed-usd-micros-yr 1000 :person (person)})
    ...
    (is (false? (:land-grant-invoked ack)))
    (is (false? (:live ack)))
    (is (= 0 (:cash-usd-micros ack)))
    (pp/assert-no-public-scores! ack)))

(deftest test-gated-no-grant
...
(deftest test-gated-receive-status-default-refuse
  (let [pkg (h/r1-dry-package {:alloc-id "a" :imputed-usd-micros-yr 1000 :person (person)})
    ...
    (is (false? (:land-grant-invoked st)))
    (is (false? (:live st)))
