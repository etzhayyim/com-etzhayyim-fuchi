...
(deftest test-package-batch-tenure-subjects
  (let [ev (couple/make-displacement-event
    ...
    (let [ts (first (:tenure-subjects gp))]
      ...
      (is (map? (:liquidity-package ts)))
      (is (= :R1-dry (get-in ts [:liquidity-package :phase])))
      (is (true? (get-in ts [:liquidity-package :member-principal])))
      (is (= 0 (get-in ts [:liquidity-package :cash-usd-micros])))
      (is (true? (:liquidity-residual-for-housing-hold ts)))
      (is (= :refused (get-in ts [:liquidity-gated-live-status :phase])))
      (is (false? (get-in ts [:liquidity-gated-live-status :loan-executed]))))
    (is (false? (:live out)))
    (is (= 0 (:cash-usd-micros out)))))
